...
class M_packages extends CI_Model {
    ...
    public function getAllPackagesWithToursAndBadges() {
        $this->db->select('bp.*, 
            ...
            (SELECT GROUP_CONCAT(tourId SEPARATOR "`") FROM tourist_attraction WHERE tourId IN (SELECT tourId FROM package_attraction WHERE packageId = bp.packageId)) as tourIds,
